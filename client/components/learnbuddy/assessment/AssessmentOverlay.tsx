      ...
      <Card className="w-full max-w-3xl border-2">
        ...
        <CardContent>
          <div className="flex gap-3 mb-4">
            <Button variant={mode === "mcq" ? "default" : "secondary"} onClick={() => setMode("mcq")}>Multiple‑choice</Button>
            <Button variant={mode === "subjective" ? "default" : "secondary"} onClick={() => setMode("subjective")}>Subjective (AI‑graded)</Button>
          </div>
            ...
            <div className="space-y-5 max-h-[50vh] overflow-auto pr-2">
              {questions.map((q, qi) => (
                <div key={qi} className="rounded-lg border p-4">
                  <div className="font-medium">Q{qi + 1}. {q.prompt}</div>
                  {q.type === "mcq" && (
                    <div className="mt-3 grid gap-2">
                      {q.options.map((opt, i) => (
                        <Button key={i} variant={selected[qi] === i ? "default" : "secondary"} className="justify-start" onClick={() => setSelected((s) => { const n = [...s]; n[qi] = i; return n; })}>{opt}</Button>
                      ))}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-3">
              ...
              {result && (
                <div className="rounded-lg border p-3 bg-secondary/50">
                  ...
                  <p className="text-sm text-foreground/70 mt-1 whitespace-pre-line">{result.feedback}</p>
                </div>
              )}
            </div>
          )}
